...
const checkURL = (input) =>
  input.startsWith("http://") && input.endsWith(".html");
...
function abbreviateFullName(fullName) {
  ...
  return `${splittedName[0].slice(0, 1)}.${splittedName[1].slice(
    0,
    1
  )}`.toUpperCase();
}
...
function arrayReduceMultiply(inputArray) {
  return inputArray.reduce((prev, curr) => {
    return prev * curr;
  }, 1);
}
...
function DNAComplemets(inputDNA) {
  ...
  return inputDNA
    .toUpperCase()
    .split("")
    .map((item, index, array) => complements[item])
    .join("");
}
...
function RGB(matrix) {
  return matrix.map((row) =>
    row.map((itemInRow) =>
      Array(3).fill(
        Math.round(itemInRow.reduce((prev, next) => prev + next, 0) / 3)
      )
    )
  );
}
...
function AddSlashInString(input) {
  return input.map((item, index, array) => {
    ...
    if (item.length % 2 == 1) {
      return (
        item.slice(0, item.length / 2 - 0.5) +
        "|" +
        item.slice(item.length / 2 + 0.5)
      );
    } else {
      return item.slice(0, item.length / 2) + "|" + item.slice(item.length / 2);
    }
...
function groupArray(inputArray) {
  ...
  for (let index = 0; index < inputArray.length; index += 3) {
    resArray.push(
      inputArray.slice(index, index + 3).reduce((prev, curr) => prev + curr, 0)
    );
  }
  return resArray;
}
